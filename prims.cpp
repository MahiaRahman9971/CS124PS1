...
float Prims(vector<vector<float>> RC) {
    // Printing RC 
    ...
    vector<float> dist(numVertices, INF);
    vector<float> stack(numVertices);
    vector<float> prev(numVertices, false);
    ...
    while (heap.heapSize() != 0) {
        ...
        for (int u = 0; u < numVertices; u++) {
            ...
            for (int v = u + 1; v < numVertices; v++) {
                // If v not in s
                float weight = computeEC(RC[u], RC[v]);
                cout << "u: " << u << endl;
                cout << "v: " << v << endl;
    ...
    cout << "totalweight at END: " << totalWeight << endl;
    return totalWeight;
};

// ERRORS:
// 1) calculates weight too many times 
// 2) doesn't work with large numbers 